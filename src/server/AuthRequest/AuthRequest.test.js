...
const FakeRequest = (headers, body) => ({ get: (header) => (headers[header]), body: body })
...
describe("AuthRequest", () => {
  describe("should not authenticate", () => {
    ...
    it("with invalid jwt", () => {
      ...
      expect(auth).toHaveProperty("isAuthenticated", false);
      expect(spy).toHaveBeenCalledWith(expect.stringMatching("Cannot find a valid JWT"));
      spy.mockReset();
      spy.mockRestore();
    });
    ...
    it("with invalid issuer jwt", () => {
      ...
      expect(auth).toHaveProperty("isAuthenticated", false);
      expect(spy).toHaveBeenCalledWith(expect.stringMatching("No public key for issuer"));
      spy.mockReset();
      spy.mockRestore();
    });
    ...
    it("with invalid subject jwt", () => {
      ...
      expect(auth).toHaveProperty("isAuthenticated", false);
      expect(spy).toHaveBeenCalledWith(expect.stringMatching("Cannot find a valid JWT with stats subject."));
      spy.mockReset();
      spy.mockRestore();
    });
  });
  ...
  it("should authenticate with valid signature", () => {
    ...
    const stamp = crypto.createHash("sha256").update(body).digest("hex");
    const auth = AuthRequest(FakeRequest({authorization: `Bearer ${MakeDebugToken({ stamp })}`}, body));
